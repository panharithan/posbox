...
class EscposDriver(Thread):
    def __init__(self, net_host, net_port):
        Thread.__init__(self)
        self.queue = Queue()
        self.lock  = Lock()
        self.host = net_host
        self.port = int(net_port)
        ...
        while True:
            try:
                ...
                if printer == None:
                    ...
                    if timestamp >= time.time() - 1 * 60 * 60:
                        #printer.print_base64_image(receipt['company']['logo'])
...
EscposDriver.current_driver = None

class EscposProxy(http.Controller):
    
    @http.route('/hw_proxy/network/open_cashbox', type='json', auth='none', cors='*')
    def open_cashbox(self, host, port):
        _logger.info('ESC/POS: OPEN CASHBOX') 
        driver = EscposDriver(host, port)
        driver.push_task('cashbox')
        
    @http.route('/hw_proxy/network/print_receipt', type='json', auth='none', cors='*')
    def print_receipt(self, host, port, receipt):
    ...
    @http.route('/hw_proxy/network/print_xml_receipt', type='json', auth='none', cors='*')
    def print_xml_receipt(self, host, port, receipt):
